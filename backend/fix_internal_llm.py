...
# 读取文件
with open('localSettingsService.js', 'r', encoding='utf-8') as f:
    content = f.read()

# 定义新的getInternalLLMSettings方法
new_method = '''    getInternalLLMSettings(userId = this.defaultUserId) {
...
# 使用正则表达式找到并替换整个getInternalLLMSettings方法
pattern = r'getInternalLLMSettings\(userId = this\.defaultUserId\) \{.*?\n    \}'
...
# 写回文件
with open('localSettingsService.js', 'w', encoding='utf-8') as f:
    f.write(content)

print("已修复getInternalLLMSettings方法")